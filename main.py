...
# ── Google Sheets setup ───────────────────────────────────────────────────
def get_sheet():
    creds_json = os.getenv('GOOGLE_CREDS_JSON', '')  # from Railway variable
    if not creds_json:
        print('WARNING: GOOGLE_CREDS_JSON not set. Skipping sheet logging.')
        return None
    ...
    return gc.open('Real Estate Lead Tracker').sheet1
 
# ── Replace log_call_outcome() with this version ──────────────────────────
async def log_call_outcome(call_id: str, phone: str, data: dict):
    ...
    hot_words  = ['interested','visit','site','haan','buy','kharidna']
    warm_words = ['later','sochta','think','mahine','baad mein']
    if any(w in transcript.lower() for w in hot_words):
        status = 'HOT'
    elif any(w in transcript.lower() for w in warm_words):
        status = 'WARM'
    ...
    if sheet:
        ...
        print(f'Logged to sheet: {phone} -> {status}')
